...
if not os.path.join( sys.path[0], 'example-dsl', 'src', 'lspExampleServer', 'server' ) in sys.path:
    sys.path.append( os.path.join( sys.path[0], 'example-dsl', 'src', 'lspExampleServer', 'server' ) )
print(sys.path)
...
from antlr.SymbolTableVisitor import SymbolTableVisitor
from antlr.DiagnosticListener import DiagnosticListener

# antlr4
if not os.path.join( sys.path[0], 'example-dsl', 'gen', 'python' ) in sys.path:
    sys.path.append( os.path.join( sys.path[0], 'example-dsl', 'gen', 'python' ) )
from antlr4 import InputStream, CommonTokenStream, Token, ParseTreeWalker
from exampleDsl.exampleDslLexer import exampleDslLexer
from exampleDsl.exampleDslParser import exampleDslParser
from exampleDsl.exampleDslVisitor import exampleDslVisitor
# antlr4-c3
if not os.path.join( sys.path[0], 'anltrLib' ) in sys.path:
    sys.path.append( os.path.join( sys.path[0], 'anltrLib' ) )
...
odsl_server = ODslLanguageServer( 'pygls-odsl-example-prototype', 'v0.1' )
...
def completions(params: Optional[CompletionParams] = None) -> CompletionList:
    ...
    parseTree: Top_levelContext = odsl_server.parser.stat()
    ...
    core.preferredRules = {exampleDslParser, exampleDslParser}

    # get completion candidates
    candidates: CandidatesCollection = core.collectCandidates( tokenIndex.index )

    if any( rule in candidates.rules for rule in [exampleDslParser.RULE_variableRef, exampleDslParser.RULE_functionRef] ):
