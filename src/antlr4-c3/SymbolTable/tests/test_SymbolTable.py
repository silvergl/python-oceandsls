...
from SymbolTable import SymbolTable, NamespaceSymbol, ClassSymbol, InterfaceSymbol, FieldSymbol, MethodSymbol, \
    BlockSymbol, VariableSymbol, FundamentalType, LiteralSymbol, DuplicateSymbolError, ScopedSymbol, SymbolTableOptions, \
...
class SymbolTableTest( IsolatedAsyncioTestCase ):
    ...
    async def test_Single_table_stress(self):
        ...
        try:
            with test_timeout(20000):
                # TODO multithreading
                # symbolTable = await self.execute(createClassSymbolTable, name= "table", counts= [300, 30, 20, 1000, 1000])
    ...
    async def test_single_table_namespace(self):
        symbolTable = await createClassSymbolTable("main", [30, 10, 10, 100, 100], ["ns1", "ns2", "ns1.ns3.ns5", "ns1.ns4.ns6.ns8"])
