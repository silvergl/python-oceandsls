...
def computeTokenPositionOfChildNode(parserRuleContext: ParserRuleContext, tokens: BufferedTokenStream, caretPosition: CaretPosition, identifierTokenTypes: List[int]):
    logger.info('start: %s > caret: %s = %s, stop: %s < caret: %s = %s',parserRuleContext.start.line, caretPosition.line, parserRuleContext.start.line > caretPosition.line,parserRuleContext.stop.line, caretPosition.line, parserRuleContext.stop.line < caretPosition.line)
    logger.info('parserRuleContext.start is not None = %s, parserRuleContext.stop is not None = %s', parserRuleContext.start is not None, parserRuleContext.stop is not None )
