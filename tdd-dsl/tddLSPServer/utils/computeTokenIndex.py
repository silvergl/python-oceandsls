...
def positionOfToken(
        token: Token, text: str, caretPosition: CaretPosition, identifierTokenTypes: List[ int ],
        parseTree: ParseTree
        ):
    start = token.column
...
def computeTokenPositionOfTerminal(
...
def computeTokenPositionOfChildNode(
        parserRuleContext: ParserRuleContext, tokens: BufferedTokenStream,
        caretPosition: CaretPosition, identifierTokenTypes: List[ int ]
        ):
    logger.info(
        'start: %s > caret: %s = %s, stop: %s < caret: %s = %s', parserRuleContext.start.line,
    ...
    if parserRuleContext.start is not None and parserRuleContext.stop is not None:
        ...
        while i <= parserRuleContext.stop.tokenIndex:
            pos = positionOfToken(
                    tokens.tokens[ i ], tokens.tokens[ i ].text, caretPosition, identifierTokenTypes,
                    parserRuleContext
                    )
